      ...
      {steps.map((step, idx) => {
        ...
        return (
          <div key={step.value} className="flex items-center">
            <div className="flex flex-col items-center gap-1">
              <div
                className={cn(
                  ...
                  isActive ? 'bg-brand-500/20 text-brand-400' : 'bg-slate-800 text-slate-600',
                  isCurrent && 'ring-2 ring-brand-500/40 ring-offset-2 ring-offset-slate-900'
                )}
              >
            ...
            {idx < steps.length - 1 && (
              <div className={cn('h-0.5 w-8 mx-1 mt-[-16px]', isActive ? 'bg-brand-500' : 'bg-slate-700')} />
            )}
          </div>
        )
      })}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h2 className="font-display text-2xl font-bold text-slate-50">{order.orderNumber}</h2>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg bg-slate-700 px-3 py-2 text-sm font-medium text-slate-200 hover:bg-slate-600 transition-colors"
          >
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-6">
          ...
          <div className="card p-5">
            ...
            {order.customerContact && <p className="text-xs text-slate-400 mt-1">{order.customerContact}</p>}
            {order.customerPhone && <p className="text-xs text-slate-400">{order.customerPhone}</p>}
            {order.customerEmail && <p className="text-xs text-slate-400">{order.customerEmail}</p>}
              ...
              <div className="flex justify-end">
                <div className="w-64 space-y-1.5">
                  <div className="flex justify-between text-base font-bold border-t border-slate-700 pt-2">
                    ...
                    <span className="text-blue-400">{formatCurrency(order.totalAmount)}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {(order.notes || order.internalNotes) && (
            <div className="card p-5 space-y-3">
              ...
              {order.internalNotes && (
                <div className={order.notes ? 'pt-3 border-t border-slate-700/50' : ''}>
                  <p className="text-xs text-slate-500 mb-1">Ghi chú nội bộ</p>
        ...
        <div className="space-y-6">
          ...
          <div className="card p-5">
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-slate-200 font-medium">{formatCurrency(order.totalAmount)}</span>
              </div>
              <div className="flex justify-between text-sm">
                  ...
                  {showPaymentForm ? (
                    <div className="space-y-2 pt-2">
                      <input
                        ...
                        placeholder="Số tiền..."
                        className="w-full rounded-lg bg-slate-900 border border-slate-700 px-3 py-2 text-sm text-slate-200 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
                      />
                      <div className="flex gap-2">
                        <button
          ...
          <div className="card p-5">
            ...
            <div className="space-y-3 text-sm">
              {order.poNumber && (
                <div>
                  <p className="text-xs text-slate-500">Số PO</p>
                  <p className="text-slate-200 font-mono">{order.poNumber}</p>
                </div>
              )}
              {order.paymentDueDate && (
                <div>
                  <p className="text-xs text-slate-500">Hạn thanh toán</p>
                  <p className="text-slate-300">{format(new Date(order.paymentDueDate), 'dd/MM/yyyy')}</p>
                </div>
              )}
              {order.expectedDelivery && (
                <div>
                  <p className="text-xs text-slate-500">Giao hàng dự kiến</p>
                  <p className={cn(
                    'text-slate-300',
              ...
              {order.actualDelivery && (
                <div>
                  <p className="text-xs text-slate-500">Giao hàng thực tế</p>
              ...
              {order.trackingNumber && (
                <div>
                  ...
                  <p className="text-slate-200 font-mono">{order.trackingNumber}</p>
                </div>
              )}
              <div>
                <p className="text-xs text-slate-500">Tiền tệ</p>
                <p className="text-slate-300">{order.currency}</p>
              </div>
            </div>
          </div>
        </div>
