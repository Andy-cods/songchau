...
// V-08: modal admin "Mời NCC" — gọi POST /vendors/invite (đã có sẵn BE) → hiện
// activation_link (email tự động đang tắt, admin gửi tay) + nút sao chép.
function InviteVendorModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  const invite = useMutation({
    mutationFn: () =>
      api.post<{ data: { activation_link?: string }; activation_link?: string }>(
        '/api/v1/procurement/vendors/invite',
        { company_name: company, contact_name: contact, email },
      ),
    onSuccess: (res: any) => {
      setLink(res?.data?.activation_link ?? res?.activation_link ?? null);
      onDone();
    },
    onError: (e: any) => toast.error(e?.detail ?? 'Lỗi mời NCC'),
  });
  ...
  const copy = async (text: string) => {
    ...
    } catch { /* clipboard bị chặn — người dùng bôi đen copy tay */ }
  };

  const inputCls =
    'w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500';

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3.5">
          <h3 className="text-base font-semibold text-slate-800">Mời nhà cung cấp</h3>
        ...
        {!link ? (
          <div className="space-y-3.5 px-5 py-4">
            <p className="text-[13px] text-slate-500">
              Tạo tài khoản NCC + link kích hoạt. Email tự động đang tắt — sao chép link ở bước sau gửi cho NCC.
            </p>
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-500">Tên công ty <span className="text-rose-500">*</span></label>
              <input value={company} onChange={(e) => setCompany(e.target.value)} className={inputCls} placeholder="Công ty TNHH ..." />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-500">Người liên hệ</label>
              <input value={contact} onChange={(e) => setContact(e.target.value)} className={inputCls} placeholder="Nguyễn Văn A" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-500">Email đăng nhập <span className="text-rose-500">*</span></label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className={inputCls + ' font-mono'} placeholder="email@congty.com" />
            </div>
            <div className="flex justify-end gap-2 pt-1">
              <button onClick={onClose} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50">Hủy</button>
              <button onClick={() => invite.mutate()} disabled={!company || !email || invite.isPending}
          ...
          <div className="space-y-3 px-5 py-4">
            <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-[13px] text-emerald-700">
              Đã tạo tài khoản NCC. Gửi link kích hoạt dưới đây cho họ (hết hạn sau 7 ngày):
            </div>
            <div className="flex items-stretch gap-2">
              <code className="flex-1 break-all rounded-md bg-slate-50 px-2.5 py-2 text-[11px] text-slate-700 ring-1 ring-inset ring-slate-200">{link}</code>
              <button onClick={() => copy(link)}
                className="inline-flex shrink-0 items-center gap-1 rounded-md bg-brand-600 px-3 text-xs font-semibold text-white hover:bg-brand-700">
                <Copy className="h-3.5 w-3.5" />{copied ? 'Đã chép' : 'Sao chép'}
              </button>
            </div>
            <div className="flex justify-end pt-1">
              <button onClick={onClose} className="rounded-lg bg-slate-800 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-900">Xong</button>
            </div>
          </div>
        )}
      </div>
    ...
    <div className={SHELL.sectionStack}>
      <FilterBar>
        ...
        <button onClick={() => setShowInvite(true)}
          className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-brand-600 px-3 text-[12px] font-semibold text-white transition-colors hover:bg-brand-700">
          <UserPlus className="h-3.5 w-3.5" /> Mời NCC
        </button>
      </FilterBar>
