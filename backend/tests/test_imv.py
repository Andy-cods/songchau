...
imvmall.com):
  ...
  4. UNIT — W3-02 (guard-on-data): imv_contracts / imv_rejections sync xong
     nhưng KHÔNG có luồng xử lý nghiệp vụ nào tiêu thụ — guard tối thiểu bắn
     Bell notification cho admin khi 1 lần sync mang new_records>0 cho 2
     entity này (_notify_admins_new_rows, gọi từ _sync_entity). Monkeypatch
     _upsert_rows + 3 helper DB như tầng 3, KHÔNG chạm notifications thật
     (giống cách _check_consecutive_errors được test — monkeypatch, không
     insert DB thật vì notifications.type là enum Postgres cần migration
     riêng mới nhận label mới).
...
──────────────────────────────────────────────────────────────────────────────
  ...
  D) Bỏ guard W3-02: xoá nhánh `elif entity in _ATTENTION_ENTITIES and ...`
     trong _sync_entity → `test_sync_entity_contracts_new_rows_notifies_admin`
     và `test_sync_entity_rejections_new_rows_notifies_admin` FAIL (notify
     không được gọi dù new_records>0).
"""
...
# ════════════════════════════════════════════════════════════════════════════
# 4. UNIT — W3-02 guard-on-data: contracts/rejections mới → cảnh báo admin
# ════════════════════════════════════════════════════════════════════════════
def _patch_sync_log_helpers(m, monkeypatch):
    """Monkeypatch 2 helper ghi log dùng chung cho mọi test _sync_entity ở đây,
    KHÔNG chạm DB thật (giống các test tầng 3 phía trên)."""
    monkeypatch.setattr(m, "_create_sync_log", lambda entity="rfq": 99)
    monkeypatch.setattr(
        m, "_update_sync_log",
        lambda log_id, result, entity_type="rfq": None,
    )
    monkeypatch.setattr(m, "_check_consecutive_errors", lambda e, msg: None)


@pytest.mark.unit
def test_sync_entity_contracts_new_rows_notifies_admin(monkeypatch):
    """entity='contracts' + new_records>0 (sync thành công) → phải gọi
    _notify_admins_new_rows('contracts', 2) đúng 1 lần — đây là guard W3-02:
    imv_contracts chưa có luồng xử lý nào khác nên phải báo người."""
    m = _imv_sync()
    _patch_sync_log_helpers(m, monkeypatch)
    monkeypatch.setattr(m, "_upsert_rows", lambda entity, rows: (2, 0))  # 2 mới, 0 cập nhật

    notified: list = []
    monkeypatch.setattr(
        m, "_notify_admins_new_rows",
        lambda entity, new_count: notified.append((entity, new_count)),
    )

    rows = [{"contract_id": "C-1", "item_code": "IT-1"}]
    result = m._sync_entity("contracts", rows)

    assert result["status"] == "success"
    assert result["new_records"] == 2
    assert notified == [("contracts", 2)]


@pytest.mark.unit
def test_sync_entity_rejections_new_rows_notifies_admin(monkeypatch):
    """entity='rejections' + new_records>0 → cùng guard áp dụng (2 entity theo
    _ATTENTION_ENTITIES, không chỉ 'contracts')."""
    m = _imv_sync()
    _patch_sync_log_helpers(m, monkeypatch)
    monkeypatch.setattr(m, "_upsert_rows", lambda entity, rows: (1, 0))
    ...
    rows = [{"rejection_id": "R-1", "item_code": "IT-9"}]
    result = m._sync_entity("rejections", rows)

    assert result["status"] == "success"
    assert result["new_records"] == 1
    assert notified == [("rejections", 1)]


@pytest.mark.unit
def test_sync_entity_contracts_zero_new_records_no_notify(monkeypatch):
    """contracts sync thành công nhưng new_records=0 (toàn bộ là cập nhật dòng
    cũ) → KHÔNG báo — guard chỉ bắn khi có dòng THỰC SỰ mới."""
    m = _imv_sync()
    _patch_sync_log_helpers(m, monkeypatch)
    monkeypatch.setattr(m, "_upsert_rows", lambda entity, rows: (0, 3))  # 0 mới, 3 cập nhật
...
@pytest.mark.unit
def test_sync_entity_other_entities_never_notify(monkeypatch):
    """entity ngoài _ATTENTION_ENTITIES (vd 'rfq') với new_records>0 → KHÔNG
    báo, dù logic guard có bug nào đó lỡ gọi chung cho mọi entity thì test
    này cũng bắt được (guard phải CÓ ĐIỀU KIỆN theo entity, không phải global)."""
    m = _imv_sync()
    assert "rfq" not in m._ATTENTION_ENTITIES  # đối chứng: rfq không nằm trong danh sách guard
    _patch_sync_log_helpers(m, monkeypatch)
    monkeypatch.setattr(m, "_upsert_rows", lambda entity, rows: (5, 0))
    ...
    rows = [{"rfq_number": "RFQ-1", "item_code": "IT-1"}]
    result = m._sync_entity("rfq", rows)
