...
class BqmsQuotePusher:
    ...
    async def _click_save_temporarily(self):
        ...
        # ── Old click-based fallback (kept for diagnostic but unreachable) ──
        for sel in [
            'button:has-text("Save Temporarily")',
            'a:has-text("Save Temporarily")',
            'input[value="Save Temporarily"]',
            'button:has-text("Save Temporary")',
            'a:text-is("Save Temporarily")',
            'a:text-is("Save Temporary")',
        ]:
            try:
                btn = scope.locator(sel).first
                if not (await btn.count() and await btn.is_visible(timeout=2000)):
                    continue

                # Capture URL + page state BEFORE click
                url_before = self._page.url
                logger.info(
                    "Pre-save state: page_url=%s edit_frame_url=%s sel=%s",
                    url_before[:120],
                    (self._edit_frame.url if self._edit_frame else "main")[:120],
                    sel,
                )

                # Click and watch for native dialog OR HTML confirm popup
                await btn.click()
                logger.info("Clicked Save Temporarily — waiting for server response")

                # Wait up to 15s for ANY response: URL change, modal, toast, native dialog
                # (native dialog auto-accepted by self._on_dialog handler)
                await asyncio.sleep(2)

                # First dump any visible modal/popup TEXT so we know what dialog
                # is asking (save-confirm? validation error? help info?)
                try:
                    popup_dump = await scope.evaluate(
                        """() => {
                            const out = [];
                            const sel = '.modal, .popup, .dialog, [class*="modal"], [class*="popup"], [class*="dialog"], [role="dialog"], [role="alertdialog"]';
                            document.querySelectorAll(sel).forEach(el => {
                                if (el.offsetParent === null) return;
                                out.push({
                                    cls: (el.className||'').slice(0,80),
                                    txt: (el.textContent||'').trim().slice(0,300),
                                });
                            });
                            return out;
                        }"""
                    ...
                    if popup_dump:
                        logger.info("Visible popups/modals: %s", popup_dump)
                    else:
                        logger.info("No visible HTML modal — likely native dialog or silent save")
                except Exception:
                    pass

                # Look for HTML confirm dialogs across ALL frames
                # BUT only inside modal-like containers (avoid clicking unrelated OK links)
                for cf_sel in [
                    '[class*="modal"]:visible button:has-text("OK")',
                    '[class*="popup"]:visible button:has-text("OK")',
                    '[role="dialog"]:visible button:has-text("OK")',
                    '[class*="modal"]:visible button:has-text("Confirm")',
                    '[class*="modal"]:visible button:has-text("Yes")',
                ]:
                    for frame in [self._page.main_frame] + [
                        f for f in self._page.frames if f != self._page.main_frame
                    ]:
                        try:
                            cb = frame.locator(cf_sel).first
                            if await cb.count() and await cb.is_visible(timeout=500):
                                txt = (await cb.text_content() or "").strip()[:50]
                                await cb.click()
                                logger.info(
                                    "Clicked modal confirm '%s' (txt=%s) in frame %s",
                                    cf_sel, txt, (frame.url or "main")[-60:],
                                )
                                await asyncio.sleep(2)
                                break
                        except Exception:
                ...
                # Wait for the form to settle / URL to change
                await asyncio.sleep(5)
                url_after = self._page.url
                logger.info(
                    "Post-save state: url_before=%s url_after=%s changed=%s",
                    url_before[-80:], url_after[-80:],
                    "YES" if url_before != url_after else "NO",
                )

                # Dump any visible alert/success/error messages
                try:
                    msgs = await scope.evaluate(
                        """() => {
                            const out = {alerts: [], any_error_text: []};
                            // Common alert/toast/message selectors
                            const sel = '.alert, .toast, .message, .msg, .notice, .success, .error, [class*="alert"], [class*="toast"], [class*="message"]';
                            document.querySelectorAll(sel).forEach(el => {
                                const t = (el.textContent || '').trim();
                                if (t && t.length < 200 && el.offsetParent !== null) {
                                    out.alerts.push({cls: (el.className||'').slice(0,50), text: t.slice(0,150)});
                                }
                            });
                            // Any visible red/error styled text
                            document.querySelectorAll('span, div, p').forEach(el => {
                                const s = window.getComputedStyle(el);
                                const t = (el.textContent || '').trim();
                                if (t && t.length < 100 && t.length > 5 &&
                                    (s.color && (s.color.includes('255, 0, 0') || s.color.includes('rgb(255'))) &&
                                    el.offsetParent !== null) {
                                    out.any_error_text.push(t.slice(0, 120));
                                }
                            });
                            return out;
                        }"""
                    )
                    logger.info("Post-save messages: %s", msgs)
                except Exception as exc:
                    logger.warning("Post-save msg dump failed: %s", str(exc)[:100])

                logger.info("Save Temporarily via %s completed", sel)
                return
            except Exception as exc:
                logger.warning("Save attempt with %s failed: %s", sel, str(exc)[:120])
                continue
        raise RuntimeError("Save Temporarily button not found")
