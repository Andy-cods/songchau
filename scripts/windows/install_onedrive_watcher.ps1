...
# - On failure: restart up to 3 times
# - Logs: %LOCALAPPDATA%\SongChauOneDriveSync\sync.log
#
# Run from PowerShell (no admin needed):
#   cd "<repo root>"
#   powershell -ExecutionPolicy Bypass -File scripts\install_onedrive_watcher.ps1
#
# Uninstall:
...
$Script   = Join-Path $RepoRoot "scripts\onedrive_continuous_sync.py"
...
if ($PyCmd) {
    ...
    if (-not $PyCmd) {
        Write-Error "Python 3 not found. Install from python.org first."
        exit 1
    }
    $PyExe  = $PyCmd.Source
...
# Ensure paramiko (and standard libs) installed
...
Register-ScheduledTask `
    ...
    -Settings $Settings `
    -Principal $Principal `
    -Description "Continuous sync OneDrive -> VPS Song Chau ERP" | Out-Null
...
Write-Host "  LastTaskResult : $($Info.LastTaskResult) (0 = OK / 267009 = currently running)"
Write-Host ""
